...
> This section is written for any Claude instance picking this up fresh. Read the entire section before doing anything. Do not re-derive what's already been decided.

### What this project is
JJK Colorizer — two-pipeline Gege Akutami manga style transfer:
- Pipeline A: any input artwork → B&W Gege manga rendering (identity-preserving)
- Pipeline B: B&W output → Gege volume cover watercolor colorization
- Stack: Illustrious XL base + two custom LoRAs + ControlNet SDXL + SD WebUI Forge inference + FastAPI backend + Next.js frontend
- Hardware: RTX 4060 Laptop 8GB VRAM, i7-12700H, 16GB RAM
- GitHub: https://github.com/roginferno17/jjk-colorizer

### What is COMPLETE
- [x] Full project structure created
- [x] SD WebUI Forge installed (forge-neo via StabilityMatrix)
- [x] Illustrious XL v0.1 (6.94GB) downloaded to Forge Stable-diffusion folder
- [x] kohya_venv310 — Python 3.10.11 venv with PyTorch 2.3.1+cu121, xformers 0.0.27, all deps
- [x] sd-scripts submodule initialized at `training/kohya_ss/sd-scripts/`
- [x] 360 BW manga panel images processed to 768x768 PNG → `dataset/bw_gege/10_gegeakutami, monochrome manga/`
- [x] 360 caption .txt files auto-generated by WD14 tagger (wd-vit-tagger-v3 ONNX) and cleaned
- [x] Color dataset: 73 PNGs at 768x768 in `dataset/color_gege/15_gegecover, painterly watercolor cover/`
- [x] FastAPI backend written (main.py, forge_api.py, preprocessing.py)
- [x] Next.js 16 frontend written (full UI, App Router, dark theme, drag-drop, polling)
- [x] All training scripts and configs written and debugged
- [x] accelerate configured: bf16, single GPU

### What is NOT done (the blockers)
- [ ] **BW LoRA not trained** — training kept hitting VRAM overflow, last config not tested yet
- [ ] Color LoRA not trained (depends on BW LoRA being validated first)
- [ ] Forge API inference not tested end-to-end
- [ ] Backend + frontend not tested together

---

### THE TRAINING PROBLEM — full history

Every training attempt failed due to VRAM overflow on RTX 4060 8GB. Here is every attempt and what it revealed:

| Attempt | Speed | What failed | Fix applied |
|---|---|---|---|
| 1 | immediate crash | Illustrious XL not downloaded | ran download_models.py |
| 2 | 275s/it | lowram missing → text encoders in VRAM | added lowram=true |
| 3 | 27s/it | num_train_epochs ignored (wrong key), resolution 1024 too big | max_train_epochs, resolution 768, Adafactor |
| 4 | 111s/it caching | vae_batch_size not set → VAE OOM during caching | vae_batch_size=1, cache_text_encoder_outputs |
| 5 | immediate crash | cache_text_encoder_outputs conflicts with text encoder LoRA training | network_train_unet_only=true |
| 6 | NOT TESTED YET | fp8_base added but never ran | — |

**Attempt 6 (fp8_base) has never been tested.** User quit before it ran. This is where things stand.

---

### CURRENT CONFIG STATE — bw_lora_config.toml

Every setting that matters:

```
resolution = 768x768          (not 1024 — saves ~1.5GB activation memory)
max_train_epochs = 15         (NOT num_train_epochs — that key is silently ignored)
train_batch_size = 1
gradient_accumulation_steps = 1   (required by fused_backward_pass assertion)
gradient_checkpointing = true
fused_backward_pass = true        (no-op in LoRA trainer but harmless)
optimizer_type = Adafactor        (NOT AdamW8bit — saves ~1.5GB optimizer states)
optimizer_args = scale_parameter=False, relative_step=False, warmup_init=False
learning_rate = 1e-4
mixed_precision = bf16            (NOT fp16, NOT fp8 — those break SDXL VAE or are invalid)
full_bf16 = true
fp8_base = true                   (UNet weights stored in fp8 → ~1.75GB savings, NEVER TESTED)
xformers = true
lowram = true                     (text encoders offloaded to CPU RAM)
max_data_loader_n_workers = 0
vae_batch_size = 1
cache_latents = true
cache_latents_to_disk = true
...
shuffle_caption = false           (required when caching text encoder outputs)
...
### WHY fp8_base SHOULD WORK

VRAM budget with all current optimizations:

| Component | VRAM |
|---|---|
| SDXL UNet weights in fp8 | ~1.75GB |
| Adafactor optimizer states | ~0.3GB |
| Activations at 768x768 + gradient checkpointing | ~1.5GB |
| Text encoders | 0GB (cached to disk, not loaded) |
| VAE | 0GB (cached to disk, not loaded after caching phase) |
| Framework overhead | ~0.4GB |
| **Total** | **~4GB** |

RTX 4060 has 8GB. 4GB headroom. This should run at **3-8s/it** with no VRAM spill.

Expected training time at 5s/it: 13500 steps × 5s = 18.75 hours (run overnight).

RTX 4060 is NOT too weak — it just requires every optimization stacked simultaneously. We have all of them now.

---

### NEXT STEP — DO THIS FIRST

1. Double-click `training\scripts\train_bw.bat`
   - Cache clears automatically (built into bat)
   - Watch for `total optimization steps: 13500` (confirms epochs correct)
   - Watch for `fp8: enabled` or similar in startup logs
   - Report it/s after first few steps complete

2. If 3-10s/it: let it run overnight. Check sample images at `training\output\bw_lora\sample\` after step 1000.

3. If still >30s/it: the remaining option is **cloud training** (see below).

4. After BW LoRA completes: pick best checkpoint (epoch 9-13 sweet spot), copy to Forge, test inference. Then train Color LoRA.

---

### FALLBACK — CLOUD TRAINING (if local still fails)

If fp8_base still doesn't fit in 8GB, train on cloud GPU and download the LoRA file:

**RunPod** (cheapest, easiest):
- URL: runpod.io
- Template: `runpod/pytorch:2.3.1-py3.11-cuda12.1.1-devel-ubuntu22.04`
- GPU: RTX 4090 (24GB) at ~$0.44/hr or A100 (40GB) at ~$1.64/hr
- At 3s/it on 4090: 13500 × 3 = 40500s ≈ 11 hours → ~$5-6 total
- Upload entire `training/` folder to pod
- Run `train_bw.bat` equivalent (same config, same sd-scripts)
- Download `training/output/bw_lora/gege_bw_lora-XXXX.safetensors` when done
- Copy to `C:\Users\Vishu\AI\StabilityMatrix\Data\Packages\forge-neo\models\Lora\`

**Google Colab T4** (free):
- T4 has 15GB VRAM — no VRAM issues at all
- Use kohya_ss Colab notebooks (search "kohya_ss colab sdxl")
- Upload config TOML + dataset ZIP

---

### KEY RULES FOR THIS PROJECT (don't violate these)

1. **Never auto-run training** — user runs all training commands, reports results
2. **Keep DEVLOG.md as running diary** — append every action, never delete history
3. **No co-author lines** in commits
4. **No mentions of "GEGEAI2.0", "krisht813", or "Claude"** anywhere in codebase
5. **SDXL resolution minimum 768** — below this breaks latent space. Never go to 512.
6. **mixed_precision stays bf16** — fp16 causes NaN in SDXL VAE. fp8 is not a valid precision mode (use fp8_base instead).
7. **max_train_epochs not num_train_epochs** — wrong key is silently ignored, defaults to 2 epochs
8. **cache_text_encoder_outputs requires network_train_unet_only=true** — otherwise crashes
9. **cache_text_encoder_outputs requires shuffle_caption=false** — pre-computed embeddings are static
...
```
Training script:    training\scripts\train_bw.bat  (double-click to run)
BW config:          training\configs\bw_lora_config.toml
Color config:       training\configs\color_lora_config.toml
BW dataset:         dataset\bw_gege\10_gegeakutami, monochrome manga\  (360 PNG + 360 TXT)
Color dataset:      dataset\color_gege\15_gegecover, painterly watercolor cover\  (73 PNG + TXT)
BW LoRA output:     training\output\bw_lora\  (checkpoints appear here)
Color LoRA output:  training\output\color_lora\
Forge models:       C:\Users\Vishu\AI\StabilityMatrix\Data\Packages\forge-neo\models\
Illustrious XL:     forge-neo\models\Stable-diffusion\Illustrious-XL-v0.1.safetensors
Python venv:        training\kohya_venv310\Scripts\python.exe
Cache cleaner:      clear_cache.py (run from project root — also auto-runs in bat)
```
